...
function refreshEpisodeSeasonVisuals(){
  document.querySelectorAll('[data-season-id]').forEach(el=>{
    ...
    if(!sid) return;
    const still = getSeasonStillByLocalSeasonId(sid);
    if(still){
      const thumb = el.querySelector('.season-card-img, .browse-card-img, .season-card-bg');
      if(thumb) thumb.style.backgroundImage = `url('${still}')`;
    }
  });
...
const Router = {
  ...
  resolve(){
    ...
    if(hash === '/' || hash === ''){
      ...
      renderMoviesPage();
    } else if(hash === '/tvshows' || hash.startsWith('/tvshows/')){
      const activeSeasonId = hash.startsWith('/tvshows/') ? hash.split('/tvshows/')[1] : null;
      ...
      renderWatchGuideV2('canon');
    } else if(hash === '/guide/release-order'){
      renderWatchGuideV2('release-order');
    } else if(hash === '/guide/black-org-guide'){
      renderWatchGuideV2('black-org-guide');
    } else if(hash === '/guide/india'){
        ...
        <div class="latest-cards-container scroll-row stagger">
          ...
          <div class="latest-cinematic-card" onclick="Router.navigate('/platform/primevideo')">
            ...
            <div class="lcc-content">
              ...
              <h2 class="lcc-title">Hindi Dub<br>Now at Ep 97</h2>
              <p class="lcc-desc">Eps 1–97 in Hindi dub — stream with Anime Times.</p>
              <div class="lcc-tags">
        ...
        <div class="manga-teaser reveal" onclick="Router.navigate('/manga')" style="cursor:pointer">
          ...
          <div class="manga-teaser-body">
            ...
            <div class="manga-teaser-btns">
              <a href="https://www.amazon.in/dp/B07JK3BJGK?binding=paperback" target="_blank" rel="noopener" class="manga-feature-btn" style="font-size:12px;padding:10px 20px">Amazon India ↗</a>
    ...
    <section id="stat-cards-section">
      <div class="section-max">
        <div class="stat-cards-grid reveal">
          ...
          <div class="stat-card">
            <div class="stat-card-num">${(()=>{const maxEp=PLATFORMS.filter(p=>p.seriesRange).reduce((m,p)=>Math.max(m,p.seriesRange[1]),0);return maxEp>0?maxEp+'<small>+</small>':'1000<small>+</small>'})()}</div>
            <div class="stat-card-label">Episodes</div>
            <div class="stat-card-sub">Across Netflix (Seasons 1–10, 23–27), Anime Times (Eps 1–96) & ETV Bal Bharat</div>
          </div>
          <div class="stat-card">
...
function renderTVShowsPage(activeSeasonId){
  if(!activeSeasonId) activeSeasonId = 'S31';
  
  app.innerHTML='';
  window.scrollTo({top:0,behavior:"instant"});
  
  const s = SEASONS.find(x => x.id === activeSeasonId);
  if(!s) { Router.navigate('/tvshows/S31'); return; }

  const pg = document.createElement('div');
  pg.className = 'page-enter';

  // Get active season's episodes
  const allEps = (typeof EPISODES !== 'undefined' ? EPISODES : []).filter(e => e.season === activeSeasonId && typeof e.n === 'number');
  ...
  pg.innerHTML = `
    <section class="movies-page-hero">
      <div class="movies-page-hero-bg" style="background-image:url('${getSeasonStillByLocalSeasonId(s.id, 4)}')"></div>
      <div class="movies-page-hero-overlay"></div>
      <div class="movies-page-hero-content">
    ...
    <section class="movies-page-body">
      <div class="section-max">
        <!-- Scrollable seasons tabs -->
        <div class="tv-season-tabs-scroll">
          ${SEASONS.map(sv => {
...
function renderYaibaPage() {
  ...
  pg.innerHTML = `
    <div class="yaiba-modal-hero" style="background-image:url('${heroImage}')">
      ...
      <button class="pp-hero-back" onclick="Router.navigate('/spinoffs')">← Back</button>
